...
class GuidelineExtracter(object):
    ...
    def funnel(self, text: str, page_id: int, lang: Language=Language.CHINESE.value):
        ...
        if self.is_chinese_abstract(text):
         ...
         #有一些识别乱码，所以中文摘要和关键词也是一个判定要素
            ...
            self.author = text if not self.author else self.author + ";" + text
            return {"is_meta": True, "meta_name": GuidelineEntities.AUTHOR.value}
    ...
    def extract(self, structure: BaseStructure):
        ...
        for element in structure:
            if element.attr in [AttrNorm.FIRST_LEVEL_TITLE.value, AttrNorm.SECOND_LEVEL_TITLE.value]:
                continue
