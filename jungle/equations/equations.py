...
from src.jungle.debug.data import dump_equation_debug_data
from src.jungle.equations.inference import get_total_texify_tokens, get_latex_batched
from src.jungle.pdf.images import render_bbox_image
from src.jungle.schema.bbox import rescale_bbox
from src.jungle.schema.page import Page
from src.jungle.schema.block import Line, Span, Block, bbox_from_lines, split_block_lines, find_insert_block
...
def insert_latex_block(page_blocks: Page, page_equation_blocks, predictions, pnum, processor):
    ...
    success_count = 0
    fail_count = 0
    for block_number, (insert_block_idx, insert_line_idx, token_count, block_text, equation_bbox) \
         in enumerate(page_equation_blocks):
        latex_text = predictions[block_number]
        conditions = [
            get_total_texify_tokens(latex_text, processor) < settings.TEXIFY_MODEL_MAX, 
...
def replace_equations(doc, pages: List[Page], texify_model, batch_multiplier=1):
    ...
    for page_idx, page_equation_blocks in enumerate(equation_blocks):
        ...
        for equation_idx, (insert_block_idx, insert_line_idx, token_count, block_text, equation_bbox) \
                in enumerate(page_equation_blocks):
