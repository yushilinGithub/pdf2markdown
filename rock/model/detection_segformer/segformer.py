...
from rock.settings import settings


def load_model(checkpoint=settings.DETECTOR_MODEL_CHECKPOINT, 
                device=settings.TORCH_DEVICE_DETECTION, 
                dtype=settings.MODEL_DTYPE_DETECTION):
    """load model"""
    ...
    if "mps" in device:
        logger.info("""Warning: MPS may have poor results. This is a bug with MPS, 
                    see here - https://github.com/pytorch/pytorch/issues/84936""")
    model = model.to(device)
    model = model.eval()
    logger.info(f"Loading detection model {checkpoint} on device {device} with dtype {dtype}")
    return model
