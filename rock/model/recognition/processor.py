...
from transformers.image_utils import PILImageResampling, \
                    ImageInput, ChannelDimension, make_list_of_images, get_image_size
import numpy as np
from PIL import Image
import PIL
from src.rock.model.recognition.tokenizer import Byt5LangTokenizer
from src.rock.settings import settings


def load_processor(checkpoint: str):
    """ load processor """
    processor = SuryaProcessor(checkpoint)
...
class SuryaImageProcessor(DonutImageProcessor):
    ...
    def process_inner(self, images: List[np.ndarray]):
        ...
        images = [SuryaImageProcessor.align_long_axis(image, size=self.max_size, 
                    input_data_format=ChannelDimension.LAST) for image in images]
        ...
        images = [
            SuryaImageProcessor.normalize(img, mean=self.image_mean, 
                    std=self.image_std, input_data_format=ChannelDimension.FIRST)
            for img in images
        ]
...
class SuryaProcessor(DonutProcessor):
    """ SuryaProcessor """
    def __init__(self, checkpoint, image_processor=None, tokenizer=None, train=False, **kwargs):
        """ init processor """
