...
from transformers.models.mbart.modeling_mbart import MBartPreTrainedModel, MBartDecoder,\
     MBartLearnedPositionalEmbedding, MBartDecoderLayer
from src.rock.model.ordering.config import MBartOrderConfig
import torch
import math
...
class MBartGQAttention(nn.Module):
    ...
    def __init__(
        ...
        assert embed_dim % self.num_kv_heads == 0, \
             f"embed_dim ({self.embed_dim}) must be divisible by num_kv_heads ({self.num_kv_heads})"
...
class MBartOrderDecoder(MBartDecoder):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_boxes, input_boxes_counts, past_key_values_length) \
                                             * self.embed_scale
