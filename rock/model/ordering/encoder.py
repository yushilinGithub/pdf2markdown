...
from transformers.models.donut.modeling_donut_swin import DonutSwinPatchEmbeddings, \
    DonutSwinEmbeddings, DonutSwinModel, \
    DonutSwinEncoder

from src.rock.model.ordering.config import VariableDonutSwinConfig
...
class VariableDonutSwinEmbeddings(DonutSwinEmbeddings):
    ...
    def forward(
        ...
        if self.row_embeddings is not None and self.column_embeddings is not None:
            ...
            row_embeddings = self.row_embeddings[:, :output_dimensions[0], :].repeat_interleave(
                                                                            output_dimensions[1], dim=1)
