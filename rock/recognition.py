...
from src.rock.postprocessing.math.latex import fix_math, contains_math
from src.rock.postprocessing.text import truncate_repetitions
...
def get_batch_size():
    """Get the batch size to use for recognition."""
...
def batch_recognition(images: List, languages: List[List[str] | None], model, \
                                    processor, batch_size=None):
    """Batch recognize text from a list of images."""
    ...
    output_text = []
    confidences = []
    logger.info(f"recognition batch size {batch_size}")
    for i in range(0, len(images), batch_size):
        ...
        for token_idx in range(len(batch_decoder_input)):
            ...
            if lang_len < max_input_length:
                batch_decoder_input[token_idx] = [processor.tokenizer.pad_id] * (max_input_length - lang_len) \
        ...
        decoder_position_ids = torch.ones_like(batch_decoder_input[0, :],
                                                 dtype=torch.int64, device=model.device).cumsum(0) - 1
        ...
        with torch.no_grad(): # inference_mode doesn't work with torch.compile
            ...
            while token_count < settings.RECOGNITION_MAX_TOKENS - 1:
                ...
                #print("batch_decoder_input", batch_decoder_input)
                ...
                decoder_position_ids = torch.ones_like(batch_decoder_input[0, :], 
                                            dtype=torch.int64, device=model.device).cumsum(0) - 1 + max_position_id
        ...
        detected_text = [fix_math(text) if math and contains_math(text)
                             else text for text, math in zip(detected_text, has_math)]
