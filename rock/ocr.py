...
from src.rock.input.processing import slice_polys_from_image, slice_bboxes_from_image
from src.rock.postprocessing.text import truncate_repetitions, sort_text_lines
from src.rock.recognition import batch_recognition
from src.rock.schema import TextLine, OCRResult


def run_recognition(images: List[Image.Image], langs: List[List[str]], 
                    rec_model, rec_processor, bboxes: List[List[List[int]]] = None, 
                    polygons: List[List[List[List[int]]]] = None, batch_size=None) -> List[OCRResult]:
    """ Polygons need to be in corner format - [[x1, y1], [x2, y2], 
    [x3, y3], [x4, y4]], bboxes in [x1, y1, x2, y2] format"""
    assert bboxes is not None or polygons is not None
    assert len(images) == len(langs), "You need to pass in one list of languages for each image"
    slice_map = []
    all_slices = []
    all_langs = []
    logger.info("start slice images for recognition ...")
    ...
    logger.info("end slice images for recognition ...")
...
def run_ocr(images: List[Image.Image], langs: List[List[str]], det_model, det_processor, 
            rec_model, rec_processor, batch_size=None) -> List[OCRResult]:
    """run ocr"""
    ...
    rec_predictions, confidence_scores = batch_recognition(all_slices, all_langs, 
                                                            rec_model, rec_processor, batch_size=batch_size)
