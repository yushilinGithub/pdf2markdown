...
from src.rock.postprocessing.fonts import get_font_path
from src.rock.schema import TextLine
from src.rock.settings import settings
from src.rock.postprocessing.math.latex import is_latex
...
def render_math(image, draw, text, s_bbox, bbox_width, bbox_height, font_path):
    """rend math"""
    try:
        from src.rock.postprocessing.math.render import latex_to_pil
...
def draw_text_on_image(bboxes, texts, image_size: Tuple[int, int],
                     langs: List[str], font_path=None, max_font_size=60, 
                    res_upscale=2, has_math=False):
    """draw text """
