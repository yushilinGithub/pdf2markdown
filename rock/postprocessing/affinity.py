...
# Scores is a tuple, one per new sequence position.  Each tuple element is bs x vocab_size
...
from src.rock.postprocessing.util import get_line_angle, rescale_bbox
from src.rock.schema import ColumnLine
...
def get_detected_lines(image, slope_tol_deg=2, vertical=False, horizontal=False) -> List[ColumnLine]:
    """
    Detect column boundaries using Hough transform on the input image
    """
...
def get_vertical_lines(image, processor_size, image_size,
                         divisor=20, x_tolerance=40, y_tolerance=20) -> List[ColumnLine]:
    """
    get vertical lines from image
    """
...
def get_horizontal_lines(affinity_map, processor_size, image_size) -> List[ColumnLine]:
    """
    get horizontal lines from affinity map
    """
